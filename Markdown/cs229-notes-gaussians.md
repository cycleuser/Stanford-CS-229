...
在最后几节中，我们主要关注如何提供一个直观的多变量高斯与对角协方差矩阵的行为。特别的，我们发现一个$n$维多元高斯对角协方差矩阵可以被简单地看作是一个集合$n$的独立系统随机变量的均值和方差分别为$\mu_i,\sigma_i^2$。在本节中，我们将更深入地探讨并提供一个当协方差矩阵不是对角时多变量高斯函数的定量解释。
