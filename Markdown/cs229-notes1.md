...
现在介绍一种非常有用的记号。指示函数（indicator function）$1\{\cdot  \}$，如果参数为真，则等于1；反之则等于0（$1{True} = 1, 1{False} = 0$）。例如1{2 = 3} = 0, 而1{3 = 5 − 2} = 1。所以我们可以把T (y) 和 y 的关系写成  (T(y))i = 1{y = i}。（往下继续阅读之前，一定要确保你理解了这里的表达式为真！）在此基础上，就有了E[(T(y))i] = P (y = i) = \phii。
